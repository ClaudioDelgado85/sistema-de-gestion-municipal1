...
const TaskFilters = () => {
  ...
  const activeFiltersText = useMemo(() => {
    const parts = [];
    if (filters.status.length > 0) parts.push(`${filters.status.length} estados`);
    if (filters.type.length > 0) parts.push(`${filters.type.length} tipos`);
    if (filters.dateRange.start) parts.push('fecha desde');
    if (filters.dateRange.end) parts.push('fecha hasta');
    return parts.join(', ');
  }, [filters.status.length, filters.type.length, filters.dateRange.start, filters.dateRange.end]);
  return (
